...
function axis1(layout, can_math_l, attach)
    ...
    ticks = cv_format_ticks("%.0f", t1...)
    ticksh = cv_format_ticks("%.1f", t2...)
    tickss = cv_format_ticks("", t3...)
    ...
    return cv_ticks_labels(layout, can_math_l, attach,
        (CV_Ruler(ticks, app), CV_Ruler(ticksh, apph), CV_Ruler(tickss, apps)))
end
...
function main()
    ...
    update_math_domains = z -> begin
        cv_paint(cc_can_math, fill_red, ec)
